  ...
  return (
    <main style={{ maxWidth: 920, margin: "8px auto", fontFamily: "sans-serif", padding: "0 8px", color: "#f5f5f5" }}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
        <button onClick={onUndo} disabled={undoStack.length === 0} style={{ padding: "6px 10px" }}>Undo</button>
        <button onClick={onRedo} disabled={redoStack.length === 0} style={{ padding: "6px 10px" }}>Redo</button>
        <span style={{ color: "#bbe7d5", alignSelf: "center", fontSize: 13 }}>{undoDiffMsg}</span>
      </div>

      <div style={{ padding: 8, borderRadius: 8, marginBottom: 8, background: "#00552e", fontSize: 14, minHeight: 92 }}>
        {resultMsg ? (
          ...
          <>
            ...
            <div style={{ minHeight: 20 }}>{selectedUke ? `メンツ手 受け入れ: ${selectedUke.mentsuKinds}種 ${selectedUke.mentsuCount}枚` : ""}</div>
            <div style={{ minHeight: 20 }}>{selectedUke ? `七対子 受け入れ: ${selectedUke.chiitoiKinds}種 ${selectedUke.chiitoiCount}枚` : ""}</div>
