    ...
    return (
      <main style={{ maxWidth: 920, margin: "8px auto", padding: "0 8px", color: "#f5f5f5", fontFamily: "sans-serif" }}>
        ...
        <div style={{ display: "grid", gap: 12 }}>
          <section style={{ display: "grid", gap: 8, background: "#00552e", borderRadius: 8, padding: 10 }}>
            <h2 style={{ margin: 0, fontSize: 16, color: "#bbe7d5" }}>通常</h2>
            <button onClick={() => setMode("random")} style={{ padding: "12px", fontWeight: 700 }}>通常配牌モード</button>
            <button onClick={() => setMode("twoShanten")} style={{ padding: "12px", fontWeight: 700 }}>二向聴チャレンジ</button>
          </section>
          <section style={{ display: "grid", gap: 8, background: "#00552e", borderRadius: 8, padding: 10 }}>
            <h2 style={{ margin: 0, fontSize: 16, color: "#bbe7d5" }}>上級</h2>
...
function GameScreen({ mode, onBackToMenu }: { mode: Mode; onBackToMenu: () => void }) {
  ...
  return (
    <main style={{ maxWidth: 920, margin: "4px auto", fontFamily: "sans-serif", padding: "0 6px", color: "#f5f5f5", minHeight: "100svh", height: "100svh", paddingBottom: "env(safe-area-inset-bottom)", display: "grid", gridTemplateRows: "auto auto auto auto auto", gap: 3, overflow: "hidden" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 6, alignItems: "start", minHeight: isDesktop ? 120 : isMini ? 68 : 78 }}>
        <River river={river} fixedHeight={isDesktop ? 180 : isMini ? 62 : 74} compact desktop={isDesktop} />
        <div style={{ display: "flex", flexDirection: "column", gap: 4, paddingTop: isMini ? 8 : 10 }}>
          <button onClick={onUndo} disabled={undoStack.length === 0} style={{ padding: "4px 8px", minWidth: isDesktop ? 74 : isMini ? 50 : 56, fontSize: isDesktop ? 14 : isMini ? 10 : 11 }}>Undo</button>
          <button onClick={onRedo} disabled={redoStack.length === 0} style={{ padding: "4px 8px", minWidth: isDesktop ? 74 : isMini ? 50 : 56, fontSize: isDesktop ? 14 : isMini ? 10 : 11 }}>Redo</button>
          <span style={{ color: "#bbe7d5", width: isDesktop ? 160 : isMini ? 84 : 96, fontSize: isDesktop ? 12 : isMini ? 8 : 9, lineHeight: 1.2 }}>{undoDiffMsg}</span>
        </div>
      </div>

      <div style={{ display: "flex", alignItems: "center", overflowX: "auto", paddingBottom: 2, minHeight: isDesktop ? 66 : isMini ? 44 : 48 }}>
        {fullHand.slice(0, 13).map((t, i) => (
          <button key={`h-${t}-${i}`} onClick={() => onTileClick(i)} style={{ width: isDesktop ? 44 : isMini ? 27 : 30, height: isDesktop ? 60 : isMini ? 38 : 42, padding: 0, borderRadius: 0, outline: i === selectedIdx ? "2px solid #6cc9ff" : "none", marginRight: -1, background: "#13523d", cursor: gameEnded ? "default" : "pointer", flex: "0 0 auto" }}>
            <MahjongTileFace tile={t} compact />
          </button>
        ))}
        {fullHand[13] != null && (
          <button key={`d-${fullHand[13]}`} onClick={() => onTileClick(13)} style={{ width: isDesktop ? 44 : isMini ? 27 : 30, height: isDesktop ? 60 : isMini ? 38 : 42, padding: 0, borderRadius: 0, outline: selectedIdx === 13 ? "2px solid #6cc9ff" : "none", marginLeft: isDesktop ? 14 : isMini ? 6 : 8, background: "#13523d", cursor: gameEnded ? "default" : "pointer", flex: "0 0 auto" }}>
            <MahjongTileFace tile={fullHand[13]} compact />
          </button>
        )}
      </div>

      <div style={{ padding: isDesktop ? 8 : isMini ? 3 : 4, borderRadius: 8, background: "#00552e", fontSize: isDesktop ? 14 : isMini ? 8 : 9, minHeight: isDesktop ? 108 : isMini ? 60 : 70, overflow: "hidden" }}>
        {resultMsg ? <div style={{ fontWeight: 700, color: "#ffe082", fontSize: isDesktop ? 22 : isMini ? 12 : 14 }}>{resultMsg}</div> : null}
      ...
      <section style={{ display: "grid", gridTemplateColumns: "repeat(5, minmax(0,1fr))", gap: isDesktop ? 6 : isMini ? 2 : 3 }}>
        <Stat compact={isMini} large={isDesktop} label="メンツ手" value={selectedIdx != null ? `${shantenM} → ${previewShantenM}` : String(shantenM)} />
        <Stat compact={isMini} large={isDesktop} label="七対子" value={shantenCLabel} />
        <Stat compact={isMini} large={isDesktop} label="巡目" value={String(turn)} />
        <Stat compact={isMini} large={isDesktop} label="良打率" value={`${goodRate}%`} />
        <Stat compact={isMini} large={isDesktop} label="勝利/総数" value={`${stats.wins}/${stats.totalGames}`} />
      </section>
    </main>
  );
}

function Stat({ label, value, compact = false, large = false }: { label: string; value: string; compact?: boolean; large?: boolean }) {
  return (
    <div style={{ border: "1px solid #2b7056", borderRadius: 8, padding: large ? 8 : compact ? 3 : 4, background: "#00552e" }}>
      <div style={{ fontSize: large ? 11 : compact ? 7 : 8, color: "#bbe7d5" }}>{label}</div>
      <div style={{ fontWeight: 700, fontSize: large ? 18 : compact ? 10 : 11, color: "#f5f5f5" }}>{value}</div>
    </div>
  );
}
...
function River({ river, fixedHeight, compact = false, desktop = false }: { river: Tile[]; fixedHeight?: number; compact?: boolean; desktop?: boolean }) {
  const rows: Tile[][] = [];
  for (let i = 0; i < river.length; i += 6) rows.push(river.slice(i, i + 6));
  return (
    <div style={{ borderRadius: 8, padding: compact ? 3 : 10, height: fixedHeight ?? 120, marginBottom: 0, background: "#00552e", display: "flex", flexDirection: "column", alignItems: "flex-start", overflowY: "auto" }}>
      {rows.length === 0 ? (
        <div style={{ color: "#bbe7d5" }}>（まだ捨て牌なし）</div>
      ) : (
        rows.map((row, rIdx) => (
          <div key={rIdx} style={{ display: "flex", gap: compact ? 2 : 8, marginBottom: compact ? 2 : 6, justifyContent: "flex-start" }}>
            {row.map((t, i) => (
              <span key={`${rIdx}-${i}`} style={{ borderRadius: 4, padding: "1px", background: "#184f3b", width: desktop ? 36 : compact ? 16 : 32, height: desktop ? 52 : compact ? 22 : 46 }}>
